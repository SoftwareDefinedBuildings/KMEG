
#include "Timer.h"
#include "../../Yggdrasil.h"

#warning ########## BAUDRATE should be 9600 in Makefile ##########

module MaxforCo2NodeP @safe(){
  provides {
    interface SensorControl as MaxforCo2NodeControl;
  }
  uses {
    // Interfaces for initialization:
    interface SplitControl as RadioControl;
    interface StdControl as RoutingControl;

    // Interfaces for communication, multihop and serial:
    interface RootControl;
    interface Send;

    // Interface for Serial
    interface StdControl as SerialControl;
    interface UartStream;

    interface Queue<message_t *> as UARTQueue;
    interface Pool<message_t> as UARTMessagePool;

    // Misc:
    interface Timer<TMilli> as Timer;
    interface Timer<TMilli> as Timer2rx;
    interface Leds;
    interface BusyWait<TMicro,uint16_t> as BusyWait;

    // GPIO Interrupt
    interface GpioInterrupt;
    interface GeneralIO;
  }
}

implementation {
  uint8_t uartlen;
  uint32_t interval;
  uint32_t timerTick;
  bool sendbusy=FALSE, uartbusy=FALSE;

  uint16_t isAlive=0;

  message_t sendbuf;
  message_t uartbuf;
  maxco2_oscilloscope_t local;

	uint8_t demo_t[2]; 
	uint8_t demo_h[2]; 
	uint8_t demo_c[2]; 
  uint8_t xorloop = 7;
  uint8_t xor_buf = 0;
  uint8_t chk_xor();
  uint8_t chk_xor_ret;
  uint16_t tmp_co2_buf = 0;
  uint16_t temp_buf = 0;
	uint16_t sensing_cnt = 0;
  task void chk_byte_send();

  static unsigned char maxco2_rd_cmd[8] = {0x7e,0x01,0x02,0x00,0x00,0x00,0x03,0x7f};

  /****************** Edited by shim *******************/
  /* Table of CRC values for high.order byte */	
  static unsigned char auchCRCHi[] = {	
    
  };	

  static unsigned char auchCRCLo[] = {
    
  };
  /****************************************************/

  uint16_t CRC16(uint8_t *puchMsg, uint16_t usDataLen);

  norace uint8_t serialBuf[80];


  /******************** BEGIN INIT ********************/
  command error_t MaxforCo2NodeControl.start(base_info_t* nodeInfo){
    memcpy(&local.info, nodeInfo, sizeof(base_info_t));   
    local.info.type = MAXCO2_OSCILLOSCOPE;
    local.info.id = TOS_NODE_ID;
    local.info.count = 0;

		demo_t[0] = 0x00;		demo_t[1] = 0xFA;
		demo_h[0] = 0x02;		demo_h[1] = 0x58;
		demo_c[0] = 0x1B;		demo_c[1] = 0x58;
    if (call RadioControl.start() != SUCCESS) {
      //call Leds.fatal_problem();
      return FAIL;
    }

    if (call RoutingControl.start() != SUCCESS) {
      //call Leds.fatal_problem();
      return FAIL;
    }

    if (call SerialControl.start() != SUCCESS) {
      //call Leds.fatal_problem();
      return FAIL;
    }else{

    }

    return SUCCESS;
  }

  command error_t MaxforCo2NodeControl.stop(){
    return SUCCESS;
  }

  event void RadioControl.startDone(error_t error) {
  //  if (error != SUCCESS)
      //call Leds.fatal_problem();
  }

  event void RadioControl.stopDone(error_t error) { }

  void setTimer(uint32_t repeat) {
    if(repeat == 0)
      interval = DEFAULT_INTERVAL;
    else
      interval = repeat;
    if (call Timer.isRunning()) call Timer.stop();
  }

  command error_t MaxforCo2NodeControl.repeatTimer(uint32_t repeat) {
    setTimer(CO2_INTERVAL);
    call Timer.startPeriodic(interval);
    return SUCCESS;	
  }

  command error_t MaxforCo2NodeControl.oneShotTimer(uint32_t repeat) {
    setTimer(repeat);
    call Timer.startOneShot(interval);
    return SUCCESS;
  }
  /********************* END INIT *********************/


  /******************** BEGIN TASK ********************/
  void task sendData() {
    // Green LED
    if (!sendbusy) {
      maxco2_oscilloscope_t *out = (maxco2_oscilloscope_t *)call Send.getPayload(&sendbuf, sizeof(maxco2_oscilloscope_t));
      if (out == NULL) {
	//call Leds.problem();
	return;
      }
      memcpy(out, &local, sizeof(local));

      if (call Send.send(&sendbuf, sizeof(local)) == SUCCESS)
	sendbusy = TRUE;
  //    else
	//call Leds.problem();
    }
    local.info.count++;
		sensing_cnt = 0;
  }
  /********************* END TASK *********************/


  /********************* BEGIN EVENT *********************/
  event void Timer2rx.fired() {
  }

  uint8_t sleepCnt = 0;
  event void Timer.fired() {
		sensing_cnt++;
    call UartStream.send(maxco2_rd_cmd, 8);  

    sleepCnt++;
    if(sleepCnt == 50){
      call SerialControl.stop();
      call SerialControl.start();
    }
    else if(sleepCnt == 60){
      WDTCTL = WDT_ARST_1_9;
    }

		if(sensing_cnt > 5){
			memcpy(&local.temp, &demo_t[0], sizeof(uint16_t));
			memcpy(&local.humi, &demo_h[0], sizeof(uint16_t));
			memcpy(&local.co2, &demo_c[0], sizeof(uint16_t));
			post sendData();
		}
  }

  /* Receive from Serial */
  uint8_t flag = 0;
  uint8_t sLen = 0;
  int16_t nTemp = 0;
  uint8_t norace rcvb = 0;

	uint8_t recvbuf[100];
	uint8_t recvpacket[10];
	uint8_t recv_cnt;
	uint8_t recv_flag = 0;
  async event void UartStream.receivedByte( uint8_t byte ) {
		if(recv_flag == 1){
    	recvbuf[recv_cnt++] = byte;

			if(byte == 0x7e){
				memcpy(recvpacket, recvbuf, 10);
				recv_flag = 0;
				recv_cnt = 0;
    		post chk_byte_send();
			}
		}
		else if(recv_cnt == 0 && byte == 0x7e){
			recv_flag = 1;
    	recvbuf[recv_cnt++] = byte;
		}
    //post chk_byte_send();
	  return;
  }

  async event void UartStream.sendDone( uint8_t* buf, uint16_t len, error_t error ) {      
    //call BusyWait.wait(10000);
    //call GeneralIO.clr();
  }

  async event void UartStream.receiveDone( uint8_t* buf, uint16_t len, error_t error ) {
  }

  async event void GpioInterrupt.fired() {
  }

  event void Send.sendDone(message_t* msg, error_t error) {
    if(error == SUCCESS) {
      if (sleepCnt > 0) sleepCnt--;
      else sleepCnt = 0;
    }
    memset(serialBuf, 0, sizeof(serialBuf));
    sendbusy = FALSE;
  }
  /********************* END EVENT *********************/

  /*********************** CRC code ********************/

  uint16_t CRC16(uint8_t *puchMsg, uint16_t usDataLen){
    uint8_t uchCRCHi = 0xff;
    uint8_t uchCRCLo = 0xff;
    uint8_t uIndex;

    while(usDataLen--){
      uIndex = uchCRCLo ^ *puchMsg++;
      uchCRCLo = uchCRCHi ^ auchCRCHi[uIndex]; 
      uchCRCHi = auchCRCLo[uIndex];
    }

    return (uchCRCLo << 8 | uchCRCHi);
  }

  uint8_t chk_xor(){
    xor_buf = 0;
    xorloop = 7;
    while (xorloop){
     xor_buf ^= recvpacket[xorloop]; 
     xorloop--;
    } 
    if (xor_buf == serialBuf[8]){
      return 1;
    }
    else {
      //serialBuf[8] = 0;
		  call Leds.led2Toggle();
      return 0;
    }
  }

  task void chk_byte_send(){
		if(recvpacket[0] != 0x7E && recvpacket[9] != 0x7E) {
		}
				call Leds.led2Toggle();
		if(recvpacket[3] == 0x01) {
			sensing_cnt++;
			// Temp, check over 8th byte
			temp_buf = recvpacket[4]*256 + recvpacket[5];
			if(recvpacket[7]==0x00){ 
				memcpy(&local.temp, &recvpacket[4], sizeof(uint16_t));
			}
			else if(recvpacket[7]==0x01) {
				temp_buf += 1000;
				memcpy(&local.temp, &temp_buf, sizeof(uint16_t));
			}
			memcpy(&demo_t[0], &recvpacket[4], sizeof(uint16_t));
			return;
		}
		else if(recvpacket[3] == 0x02) {
			sensing_cnt++;
			// Humi, check over 8th byte
			memcpy(&local.humi, &recvpacket[4], sizeof(uint16_t));
			memcpy(&demo_h[0], &recvpacket[4], sizeof(uint16_t));
			return;
		}
		else if(recvpacket[3] == 0x03) {
			sensing_cnt++;
   		// CO2, check over 5th byte
			if (recvpacket[7]==0x02) {
				tmp_co2_buf = 5;
			}
			else if (recvpacket[7]==0x00) 
				tmp_co2_buf = recvpacket[4]*255 + recvpacket[5];
			else tmp_co2_buf = 55;
				memcpy(&local.co2, &recvpacket[4], sizeof(uint16_t));
			memcpy(&demo_c[0], &recvpacket[4], sizeof(uint16_t));
		}

		if (recvpacket[3]==0x03){
			chk_xor_ret = chk_xor();
			if(chk_xor_ret==1){
				call Leds.led1Toggle();
				post sendData();
			}
			else {
				call Leds.led0Toggle();
			}
		}
	}
/*
  task void chk_byte_send(){
      if (sLen==0) { 
        serialBuf[0]=0x4F;
        flag=0;
      }

      serialBuf[sLen] = rcvb;
      sLen++;

      if(serialBuf[0] == 0x7E && sLen == 2) { // chack 1st, 2nd serial received byte
	      if(serialBuf[1] == 0x01) {
	         sLen = 2;
           flag=1;
	      }
        else  sLen = 0; 
	      return;
      }

      if (!flag) return;

     if(serialBuf[0] == 0x7e && serialBuf[1] == 0x01 && sLen == 3) {
     // check 3rd byte
	     if(serialBuf[2] != 0x01) {
         sLen = 0;
         flag = 0;
         return;
   	   }
     }

     else if(serialBuf[2] == 0x01 && sLen == 4) {
     // check 4th byte
	     if(!(rcvb == 0x01 || rcvb == 0x02 || rcvb == 0x03)) {
         sLen=0;
         flag = 0;
         return;
   	   }
     }

     else if(serialBuf[3] == 0x01 && sLen == 8) {
				sensing_cnt++;
     // Temp, check over 8th byte
			 temp_buf = serialBuf[4]*256 + serialBuf[5];
	     if(serialBuf[7]==0x00){ 
		     memcpy(&local.temp, &serialBuf[4], sizeof(uint16_t));
		   }
		   else if(serialBuf[7]==0x01) {
			   temp_buf += 1000;
		     memcpy(&local.temp, &temp_buf, sizeof(uint16_t));
		   }
       else if (serialBuf[4] > 2) ;//call Leds.led2Toggle();

				memcpy(&demo_t[0], &serialBuf[4], sizeof(uint16_t));

       // if you modify, serialBuf value, it causes XOR check error 
       return;
     }

     else if(serialBuf[3] == 0x02 && sLen == 8) {
				sensing_cnt++;
     // Humi, check over 8th byte
	     memcpy(&local.humi, &serialBuf[4], sizeof(uint16_t));
				memcpy(&demo_h[0], &serialBuf[4], sizeof(uint16_t));
       return;
     }

     else if(serialBuf[3] == 0x03 && sLen == 8) {
				sensing_cnt++;
     // CO2, check over 5th byte
       if (serialBuf[7]==0x02) {
         tmp_co2_buf = 5;
       }
       else if (serialBuf[7]==0x00) 
         tmp_co2_buf = serialBuf[4]*255 + serialBuf[5];
       else tmp_co2_buf = 55;
					memcpy(&local.co2, &serialBuf[4], sizeof(uint16_t));
				memcpy(&demo_c[0], &serialBuf[4], sizeof(uint16_t));
       return;
     }
     
			if (sLen == 10){
       chk_xor_ret = chk_xor();

       if (serialBuf[3]==0x03 && chk_xor_ret==1) {
         post sendData();
       }

       sLen = 0;
       flag = 0;

       if (chk_xor_ret != 1) {
         // XOR Error - invalid serial packet received
         // green LED of MAXFOR CO2 NODE
         return;
       }

     }
   
     // ensure that sLen shuold be within 10
     if (sLen > 10) { sLen = 0; flag = 0; }
  }
*/
}
