...
implementation {
  ...
  void TestFuc(csi_oscilloscope_t * packet){
    ...
    call SubSend.send(TOS_BCAST_ADDR , &sendbuf, sizeof( splug_oscilloscope_t ));
  }
  ...
  event void WatchCat.fired(){
    ...
    else if(local.type == PIR_OSCILLOSCOPE){
    
      pir_oscilloscope_t *out;
      out = (pir_oscilloscope_t *)call SubSend.getPayload(&sendbuf , sizeof(pir_oscilloscope_t));
      out->interrupt = 0;
      call SubSend.send(TOS_BCAST_ADDR , &sendbuf, sizeof( pir_oscilloscope_t ));
    }
