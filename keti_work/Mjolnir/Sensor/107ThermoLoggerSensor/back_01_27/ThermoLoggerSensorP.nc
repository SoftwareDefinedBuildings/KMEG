...
#include "Timer.h"
#include "../../Yggdrasil.h"
#define ADC_PORT_COUNT 7
#define ADC_DATA_COUNT 7
...
implementation {
	typedef struct gAdcData {
		uint8_t cur;
		uint16_t data[ADC_DATA_COUNT];
	} gAdcData_t;
	
	void CollectData(gAdcData_t *adcData, uint16_t data); 
  ...
  thermo_logger_oscilloscope_t local;
	gAdcData_t gAdcData[ADC_PORT_COUNT];
	
	//uint8_t gCurrentAdc=0, gAdcCount=0;
	//uint16_t gAllData[8][DATA_COUNT];
